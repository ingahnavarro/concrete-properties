"""ACI318_19 class for designing to the Costa Rican Seismic Code 2010/14."""
...
import anm_module.materials_constitutive_models as anm_materials
...
class ACI318_19(DesignCode):
    """Design code class for ACI 318-19 and CSCR 2010/14.
    ...
    def create_concrete_material(
        ...
        r"""Returns a concrete material object to ACI318_19.

        .. admonition:: Material assumptions

          - *Density*: 2400 kg/m3

          - *Elastic modulus*: Interpolated from ACI 19.2.2.1
          ...
          - *Alpha squash*: ACI 318-19 22.2.2.4.1

          - *Flexural tensile strength*: From ACI 318-19 19.2.3.1
        ...
        Raises:
            ValueError: If ``compressive_strength`` is not between 210 kgf/cm2 and 1000 kgf/cm2.

        Returns:
            Concrete material object
        """
        if compressive_strength < 210 or compressive_strength > 1000:
            raise ValueError("compressive_strength must be between 210 kgf/cm2 and 1000 kgf/cm2.")

        # create concrete name
        name = f"{compressive_strength:.0f} kgf/cm2 Concrete (ACI318-19)"

        # calculate elastic modulus
        elastic_modulus = 15100 * np.sqrt(compressive_strength)

        # calculate stress block parameters
        alpha = 0.85  # 22.2.2.4.1
        gamma = anm_materials.beta_1_CSCR(compressive_strength)

        # calculate flexural_tensile_strength
        flexural_tensile_strength = 2 * np.sqrt(compressive_strength)
    ...
    def create_steel_material(
        ...
        .. admonition:: Material assumptions
          ...
          - *Elastic modulus*: 2100000 kgf/cm2
        ...
        if ductility_class == "N":
            fracture_strain = 0.06 # ACI 318-19 T20.2.1.3(c)
        ...
        return SteelBar(
            name=f"{yield_strength:.0f} kgf/cm2 Steel (ACI 318-19)",
            density=7.85e-3,
            stress_strain_profile=ssp.SteelElasticPlastic(
                yield_strength=yield_strength,
                elastic_modulus=2.10e6,
    ...
    def capacity_reduction_factor(
        ...
        """Returns the ACI 318-19 capacity reduction factor 21.2.2
    ...
    def ultimate_bending_capacity(
        ...
        r"""Calculates the ultimate bending capacity with capacity factors to ACI 318-19.

        Args:
            theta: Angle (in radians) the neutral axis makes with the horizontal axis
                (-pi <= theta <= pi)
            n_design: Design axial force, N*
            phi_0: Compression dominant capacity reduction factor, see  21.2.2
    ...
    def moment_interaction_diagram(
        ...
        r"""Generates a moment interaction diagram with capacity factors to ACI 318-19.
        ...
        .. note::
            ...
            to be the unfactored net (nominal) axial load N* / phi.
    ...
    def biaxial_bending_diagram(
        ...
        """Generates a biaxial bending with capacity factors to ACI 318-19.

        Args:
            n_design: Design axial force, N*
            n_points: Number of calculation points
            phi_0: Compression dominant capacity reduction factor, 21.2.2
            progress_bar: If set to True, displays the progress bar
